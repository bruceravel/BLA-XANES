...
specified at the command line or via the ~BALCONFIG~ environment
...
The possible steps to mask creation are:
 ...
 6. Use the entire image.  This step just sets all the pixels in the
    mask to 1 so that the entire image is used to compute the energy
    point.  Note that it makes no sense to use this step with any step
    other than the bad/weak step, which should precede this step.
