...
:     --incident | -i    [number]  incident energy (required for xes task)
...
files, and the ~-c~, ~-e~, ~-i~, and ~-a~ command line options are
demonstrated.
...
Use of the ~-i~ flag can be avoided by setting the ~BLAINCIDENT~
environment variable.  The following are equivalent:

:   bla herfd -c=/path/to/config.ini -i=11970 -a Aufoil1
: 
:   export BLAINCIDENT=11970
:   bla herfd -c=/path/to/config.ini -a Aufoil1
