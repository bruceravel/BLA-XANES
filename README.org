...
Since the ~mask~ and ~point~ tasks are intended for use during data
acquisition, verbosity is tunred off regardless of the command line
switches for the ~mask~ and ~point~ tasks.  Saving is turned on for
the ~mask~ task regardless of the command line switches.
