...
At line 3116 of ~PDL::Graphics::Gnuplot~, add the following line:
...
Similar at lines 3757, 3301.

This suppresses the ~Reading ras files from sequential devices not
supported~ warning when using the qt terminal.  This is a qt bug and
appears to be of no consequence.

(It's ~/usr/local/share/perl/5.20.2/PDL/Graphics/Gnuplot.pm~ on my
laptop.)
